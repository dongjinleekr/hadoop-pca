...
import com.github.dongjinleekr.hadoop.covariance.CovarianceJob;
...
public class VectorMeanJob extends AbstractJob {
  ...
  public int run(Configuration conf, Path input, Path output, int numRows, int numCols) throws IOException, ClassNotFoundException, InterruptedException {
    ...
    MultithreadedMapper.setMapperClass(job, VectorMean.MeanMapper.class);
    MultithreadedMapper.setNumberOfThreads(job, threadCount);
    job.setReducerClass(VectorMean.MeanReducer.class);
